...
- **`_posts/` is an Obsidian vault.** `.obsidian/` is committed, and the obsidian-git plugin auto-commits a "vault backup" every few minutes — frequent auto-commits are normal, and every push to `main` triggers a site deploy. Posts are `_posts/YYYY-MM-DD-<title>.md` with `title` / `date` (+0800 offset) / `categories` / `tags` front matter; permalinks are `/posts/:title/`. Existing posts contain Obsidian syntax (wikilinks `![[...]]`, `Pasted image ...` filenames) that does not render on the site — use plain Jekyll Markdown when writing posts via Claude.
...
- Site images (e.g. the sidebar avatar `assets/img/1.jpg`) live in `assets/img/`; images embedded in posts are stored alongside the posts under `_posts/`.
